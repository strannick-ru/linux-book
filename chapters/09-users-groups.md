...
Но этого недостаточно. Каталог проекта находится внутри вашего
...
Эта команда добавляет только право на **проход** (`x`) для остальных
...
> разрешает проход через него (то есть обращение к файлам внутри по
> полному пути). Бит `r` разрешает чтение списка файлов. Мы добавили
> только `x`: webmaster сможет попасть в `wordpress-project`, но не
> сможет просмотреть, что ещё лежит в вашем домашнем каталоге.
> Это стандартная практика, когда нужно предоставить доступ к вложенному
> каталогу, не раскрывая всё содержимое.
