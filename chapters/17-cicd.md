...
> но в продакшене стоит рассмотреть более изолированные варианты
> (см. врезку ниже).

---

> ### Врезка. DooD и DinD — два способа дать CI/CD доступ к Docker
>
> Когда Runner с Docker-executor выполняет задачу, он запускает
> контейнер. Но что если задаче самой нужно вызывать `docker` —
> собирать образы или запускать `docker compose`? Контейнер
> по умолчанию не имеет доступа к Docker. Есть два подхода.
>
> **Docker-outside-of-Docker (DooD)** — монтирование сокета хоста:
>
> ```toml
...
> ```
>
> Контейнер Runner подключается к Docker-демону, который работает
> на хосте. Команды `docker build`, `docker compose up` выполняются
> тем же демоном, что обслуживает и сам Runner. Контейнеры,
> созданные из задачи, — это обычные контейнеры на хосте,
> они видят друг друга и делят ресурсы с остальными.
>
> *Плюсы:* просто настроить, быстро работает (нет накладных
> расходов на вложенный демон), образы кешируются на хосте.
>
> *Минусы:* задача получает полный доступ к Docker на хосте —
> может увидеть и удалить чужие контейнеры. Для доверенного
> учебного сервера это приемлемо, для общего CI-кластера — нет.
>
> **Docker-in-Docker (DinD)** — отдельный демон внутри контейнера:
>
> ```toml
> privileged = true
> ```
...
> ```
>
> Runner запускает рядом с задачей дополнительный контейнер
> (`docker:27-dind`), в котором работает собственный Docker-демон.
> Задача обращается к нему по сети, а не через сокет хоста.
> Каждый запуск конвейера получает чистый Docker — контейнеры
> и образы от предыдущих запусков не остаются.
>
> *Плюсы:* полная изоляция — задача не видит контейнеры хоста.
>
> *Минусы:* требует `privileged: true` (контейнеру разрешается
> управлять ядром), образы не кешируются между запусками
> (каждый раз скачиваются заново), сложнее в настройке.
>
> **Что мы используем в курсе.** DooD — монтирование сокета.
> Для учебного проекта с одним Runner на собственном сервере
> это самый простой и быстрый вариант. В продакшене с общими
> Runner чаще выбирают DinD или rootless Docker — чтобы задачи
> разных проектов не могли влиять друг на друга.
